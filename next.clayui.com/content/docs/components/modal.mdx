...
- **observer** - Observer is the communication channel that connects ClayModal with `useModal`, you can call it a tunnel.
...
To prevent Modal from closing before animation never set its state directly in button events to close the modal instead call the `onClose` method returned by `useModal`.
...
`UseModal` is a hook so it implies that you cannot use ClayModal in a class component, to use you can adopt some strategies:

- Wrap ClayModal and `useModal` into one component.
- Create your modal expert on a functional component.
...
When your application uses a lot of modals, it may be bad to be creating multiple modes for your application, you can use `<ClayModalProvider />` to have only one open Modal on the screen and the component centered on your application, you can invoke the modal from the context through the API.
