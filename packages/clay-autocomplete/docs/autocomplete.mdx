...
Autocomplete supports providing visual feedback to users about which items are in a selected state by using the `selectedKeys` prop. This prop accepts an array of keys corresponding to the items that should be marked as selected with a checkmark indicator.

**Important**: The `selectedKeys` prop only provides visual feedback and does not control the actual value of the input. It works only in a controlled manner and must be managed by the parent component.
...
export default function App() {
	const [value, setValue] = useState('');
	const selectedItems = ['Bananas', 'Oranges'];
	...
	return (
		<Provider spritemap="/public/icons.svg">
			<div className="p-4">
				<Form.Group>
					<label
						htmlFor="clay-autocomplete-selected"
						id="clay-autocomplete-label-selected"
					>
						Fruits (Bananas and Oranges are pre-selected)
					</label>
					<Autocomplete
						aria-labelledby="clay-autocomplete-label-selected"
						id="clay-autocomplete-selected"
						defaultItems={[
							'Apples',
							'Bananas',
							'Cantaloupe',
...
This is particularly useful when the Autocomplete is used in scenarios where multiple selections have been made elsewhere in your application, and you want to show users which items are already selected without changing the input value.
