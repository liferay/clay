...
module.exports = function(gulp, plugins, _, config) {
	...
	var TPL_SVG =
		'<svg class="lexicon-icon lexicon-icon-{0}"><use xlink:href="{{rootPath}}/images/icons/icons.svg#{0}" /></svg>';
	...
	gulp.task('build:metalsmith', function(cb) {
		...
		var svgFilter = plugins.filter(
			['content/icons-lexicon.html'],
			filterConfig
		);
		var changelogFilter = plugins.filter(['./CHANGELOG.md'], filterConfig);

		var svgFiles = gulp
			.src(['src/images/icons/*.svg', '!src/images/icons/flags-*.svg'], {
				read: false,
			})
			.pipe(
		...
		var svgFlags = gulp
			.src(['src/images/icons/flags-*.svg'], {read: false})
			.pipe(
				plugins.rename(function(file) {
					file.basename = _s.slugify(
						file.basename.replace(svgstore.REGEX_FLAGS, '')
					);
				})
			);

		return gulp
			.src(['./CHANGELOG.md', config.SRC_GLOB])
			.pipe(filter)
	...
	gulp.task('build:sassdoc', function() {
		...
		return gulp
			.src(['./src/scss/**/**/*', '!./src/scss/bootstrap/**/*'])
