...
<div className="clay-site-alert alert alert-info">
  ...
  by the <code className="language-text">key</code> property configured in the <code className="language-text">
    Option
  </code> component, so the component can identify which value is selected and which
  should be shown in the trigger. <br />
  <br />
              ...
              {(item) => (
                <Option key={item} textValue={item}>
                  <Layout.ContentRow>
                    ...
                    <Layout.ContentCol>
                      <Label aria-hidden displayType="success">
                        Active
                      </Label>
                    </Layout.ContentCol>
                  </Layout.ContentRow>
