...
const ChartBase = {
	...
	attached() {
		...
		this.resolveData_(data).then(data => {
			...
			this.on('dataChanged', this.handleDataChanged_.bind(this));
	 ...
	 * @protected
	 */
	constructDataConfig_(attachListeners = true) {
		const state = this.getStateObj_();
