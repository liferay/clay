...
import {isDefAndNotNull, isNumber, isObject} from 'metal';
...
class PredictiveChart extends Chart {
	...
	_normalizeData() {
		for (let i = 0, l = this.data.length; i < l; i++) {
			...
			if (el.id !== 'x') {
				for (let i = 0, l = el.data.length; i < l; i++) {
					if (
						(!Array.isArray(el.data[i]) || !isObject(el.data[i])) &&
						isNumber(el.data[i])
					) {
						el.data[i] = {
							low: el.data[i],
...
PredictiveChart.STATE = {
	 ...
	 */
	predictionDate: {
		validator: val => val instanceof Date,
	},
